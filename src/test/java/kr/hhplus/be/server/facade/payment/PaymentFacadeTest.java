...
public class PaymentFacadeTest {
    ...
    @Test
    @DisplayName("결제 리스트 조회 테스트")
    void testGetPaymentListByUser() {
        // Given
        String userId = "user1";
        Pageable pageable = Pageable.ofSize(10);

        List<PaymentDomainDto> paymentDomainDtos = List.of(
                new PaymentDomainDto(1L, 101L, 100.0, PaymentStatus.COMPLETED, LocalDateTime.now().minusDays(1), LocalDateTime.now().minusDays(1)),
                new PaymentDomainDto(2L, 102L, 150.0, PaymentStatus.PENDING, LocalDateTime.now().minusDays(3), LocalDateTime.now().minusDays(3))
        );

        Page<PaymentDomainDto> paymentPage = new PageImpl<>(paymentDomainDtos);
        when(paymentService.getPaymentsByUser(userId, pageable)).thenReturn(paymentPage);

        // When
        List<PaymentResponseDto> result = paymentFacade.getPaymentListByUser(userId, pageable);

        // Then
        verify(paymentService).getPaymentsByUser(userId, pageable);

        assertThat(result).hasSize(2);
        assertThat(result.get(0).orderId()).isEqualTo(101);
        assertThat(result.get(0).status()).isEqualTo("COMPLETED");
        assertThat(result.get(1).orderId()).isEqualTo(102);
